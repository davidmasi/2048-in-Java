...
public class Board {
    ...
    public void addRandomTile() {
        ...
        BoardSpot randomSpot = openSpaces.get(t);
        int row = randomSpot.getRow();
        int col = randomSpot.getCol();
        while (gameBoard[row][col] != 0) {
            t = StdRandom.uniform(openSpaces.size());
            randomSpot = openSpaces.get(t);
            row = randomSpot.getRow();
            col = randomSpot.getCol();
        }
    ...
    public void swipeLeft() {
        for (int i = 0; i < gameBoard.length; i++) {
            ...
            for (int j = 0; j < gameBoard[i].length; j++) {
                if (gameBoard[i][j] != 0) {
                    gameBoard[i][nextNonZero] = gameBoard[i][j];
                    nextNonZero++;
                }
            }
            for (int j = nextNonZero; j < gameBoard[i].length; j++) {
                gameBoard[i][j] = 0;
            }
        }
    }
    ...
    public void transpose() {
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = i + 1; j < gameBoard[0].length; j++) {
    ...
    public void flipRows() {
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard[0].length / 2; j++) {
                int temp = gameBoard[i][j];
                gameBoard[i][j] = gameBoard[i][gameBoard.length - 1 - j];
                gameBoard[i][gameBoard.length - 1 - j] = temp;
            }
        }       
    }          
